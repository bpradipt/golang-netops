...
func OpenNamespaceByName(name string) (Namespace, error) {

	nsHandle, err := netns.GetFromName(name)
	if err != nil {
		return nil, fmt.Errorf("failed to get network namespace %s: %w", name, err)
	}

	handle, err := netlink.NewHandleAt(nsHandle)
	if err != nil {
		return nil, fmt.Errorf("failed to get a handle for network namespace %s: %w", name, err)
	}
